...
class PhasicTonic():
    ...
    def fit(self, eeg, hypno):
        ...
        rem_epochs = get_rem_epochs(eeg.to_numpy(), hypno, self.fs)
    ...
    def plot(self):
        ...
        freq, t, SP = spectrogram(data=self.eeg, fs=self.fs, window='hann', 
        ...
        for epoch in self.rem_interval:
            rem_start, rem_end = epoch["start"].item(), epoch["end"].item()
            axs["lfp"].axvspan(rem_start, rem_end, facecolor=[0.7, 0.7, 0.8], alpha=0.4)
            
            tridx = self.tridx_seq[(rem_start, rem_end)] 
            sdiff = self.sdiff_seq[(rem_start, rem_end)]
            eegh = self.eeg_seq[(rem_start, rem_end)]
            
            tridx = tridx/self.fs + rem_start
